...
async function computeAvailability(roomId) {
    ...
    for (const day of days) {
        ...
        while (slotStart.getTime() + meetingDuration * 60 * 1000 <= dayEnd.getTime()) {
            ...
            for (const participant of participants) {
                ...
                const unavailableDays = user.unavailableDays || [];
                ...
                if (slotMinutes < prefStartMin || slotEndMinutes > prefEndMin)
                    continue;
                if (user.sleepStartHour != null && user.sleepEndHour != null) {
                    ...
                    if (sleepStartMin > sleepEndMin) {
                        if (slotMinutes >= sleepStartMin || slotEndMinutes <= sleepEndMin)
                            continue;
                    }
                    else {
                        if (slotMinutes >= sleepStartMin && slotEndMinutes <= sleepEndMin)
    ...
    await index_1.prisma.suggestion.deleteMany({ where: { roomId } });
    await index_1.prisma.suggestion.createMany({
...
async function getRoomStats(roomId) {
    ...
    for (const s of suggestions) {
        ...
        if (!dayCounts[day])
            dayCounts[day] = { count: 0, totalScore: 0 };
        dayCounts[day].count++;
